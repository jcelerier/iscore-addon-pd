...
struct Node
{
  ...
  static const constexpr auto info =
      ...
      .controls(Process::FloatSlider{"Freq.", 0., 50., 1.}
              ...
              , Process::Enum<7>{"Function", 0,
                  {"Sin", "Triangle", "Saw", "Square", "Noise 1", "Noise 2", "Noise 3"}
                }
                )
      .state<State>()
      .build();
  ...
  static void run_precise(
      float freq, float coarse, float fine, float jitter, float phase, const std::string& type,
      ...
      ossia::execution_state&)
  {
    ...
    {
      float new_val{};
      const auto phi = 2.f * float(M_PI) * freq * s.phase / 44100.f;
      switch(it->second)
      {
        case Sin:
